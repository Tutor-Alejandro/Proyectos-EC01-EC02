...
from app.eda_statistics import mostrar_tests
...
def generador_graficos(df):
    ...
    if tipo == "Histogram":
        ...
        # Verificar si la variable seleccionada es numérica (continua)
        if df[col_x].dtype == 'float64':
            fig = px.histogram(
                df,
                x=col_x,
                color=target_col if target_col else None,
                nbins=30,
                histnorm='density',   # Normaliza para mostrar densidad
                opacity=0.6,
                title=f"Densidad de {col_x}"
            )
            fig.update_traces(marker_line_width=0.5)
        else:
            # Si es categórica, usa histograma normal
            fig = px.histogram(
                df,
                x=col_x,
                color=target_col if target_col else None,
...
def ejecutar_eda_2(df):
    ...
    mostrar_kpis(df, target_col=target)
